...
**Status:** design, approved in conversation 2026-09-13. Spec and implementation land on `feature/browser-automation-adapter`.
...
- **A subscription resolver cannot see the client's host.** DGS 9.2.2's `graphql-transport-ws` handler (the protocol the agent speaks via `graphql-ws` 6) calls `DgsQueryExecutor.execute(query, variables, extensions, headers = null, operationName, webRequest = null)`, and `RequestContextHolder` is empty on the WebSocket thread. The DGS `Context` holds the `connection_init` params but is private to the protocol handler.
- **The handshake can.** The HTTP upgrade request is a servlet request with the socket address; `AuthenticationHttpSessionHandshakeInterceptor` is already registered on `/subscriptions` and does nothing.
- **The resolver runs on the WebSocket thread.** `DgsWebSocketHandler.handleTextMessage` calls the protocol handler inline, `DefaultDgsQueryExecutor.execute` blocks on `CompletableFuture.get()`, and `registerAgent` is not `suspend`. A value bound to the thread while a message is handled is visible to the resolver. This is DGS behaviour, not contract, so an integration test pins it.
...
- The handshake interceptor, the WebSocket decorator and `HostAllowlist` in `server-core`. The adapter only ever receives `trusted: Boolean`.
...
## Trust: handshake to `openInstance`
...
**Matcher.** `HostAllowlist` in `server-core`, package `session`: `matches(address: InetAddress): Boolean`. IPs and CIDR ranges are matched directly; hostnames are resolved with `InetAddress.getAllByName` on every check, uncached, because handshakes are rare. A name that does not resolve does not match and logs a warning; it never throws. `StatefulAuthService.isWhitelisted` switches to a `HostAllowlist` built from `app.whitelistedHosts` plus the loopback and local addresses it adds today, which drops the startup resolution from throttling without changing its config. `StatelessAuthService.isWhitelisted` stays `true`.

**Handshake.** `AuthenticationHttpSessionHandshakeInterceptor` becomes an injected bean. It reads the socket address (`ServerHttpRequest.remoteAddress`), never `X-Forwarded-For` or `X-Real-IP`, and stores `agentTrusted = allowlist.matches(address)` in the handshake attributes. It never rejects.

**To the resolver.** A `WebSocketHandlerDecorator` wraps `DgsWebSocketHandler` in `CustomDgsWebSocketConfig`. For each message it binds the session's `agentTrusted` to the ThreadLocal holder `AgentConnection` (`graphql-api`, package `browserautomation`) and clears it in `finally`. Unbound, `AgentConnection.isTrusted()` returns `false`. `BrowserAutomationResolver.registerAgent` reads it on its first line, before `runBlocking`, and calls `gateway.registerAgent(data, trusted)`.

**In the adapter.** `BrowserAutomationRef` gains `openInstance`; `addAgent` saves `openInstance = agentRef.openInstance` instead of `true`.
...
- Interceptor test (unit, `server-core`): a request from a non-allowlisted socket address with spoofed `X-Real-IP` and `X-Forwarded-For` headers is stored as untrusted; an allowlisted socket address as trusted.
...
- WebSocket integration test (`server-core`, random port): a `graphql-transport-ws` client over loopback subscribes to `registerAgent`; the mocked `BrowserAutomationGateway` receives `trusted = true` when `trustedHosts` contains `127.0.0.1,::1` and `false` when it is empty. This is the test that fails if DGS stops calling the resolver on the WebSocket thread.
...
- Rejecting untrusted hosts at the handshake.
- Fixing `HttpUtil.getRemoteAddr`'s trust in `X-Real-IP` for throttling.

## Risks

- The thread-bound handoff depends on DGS internals; the WebSocket integration test is the guard, and a DGS upgrade that breaks it fails CI rather than silently untrusting agents.
