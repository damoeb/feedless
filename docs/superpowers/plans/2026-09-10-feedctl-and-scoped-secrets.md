...
- The prerender worker's image is now `damoeb/feedless:browserautomation-*` (was `agent-*`), and its k8s Deployment and Service are `feedless-browserautomation` (was `feedless-agent`); after the first deploy remove the old ones once with `kubectl delete deployment,service feedless-agent`.
- Docker Hub has the `browserautomation-*` tags only after the next image push; until then `docker compose pull` with `docker-compose.prod.yml` fails for the `feedless-browserautomation` service.
