...
- Docker Hub has the `browser-automation-app-*` tags only after the next image push; until then `docker compose pull` with `docker-compose.yml` or `docker-compose.prod.yml` fails for the `feedless-browser-automation-app` service.
- The docker-compose service is now `feedless-browser-automation-app` (was `feedless-agent`); compose deployments run `docker compose up -d --remove-orphans` once, otherwise the old `feedless-agent` container keeps running next to the new one.
