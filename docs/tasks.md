...
- [ ] **`FeedService.requireLegacyTokenAccess` überspringt die Zugriffsprüfung, wenn `resolveClaim` null liefert** (ungültiges Token oder ein gültig signiertes Token, dessen `RepositoryClaim`-Zeile fehlt, z. B. nach einem Datenbank-Reset — vergleichbar mit dem bereits akzeptierten `userExists`-Fall in `SessionResolver`). Heute unschädlich, weil `resolveFeed` denselben Nullwert prüft und in diesem Fall ebenfalls auf den öffentlichen URL-Pfad ausweicht statt das beanspruchte Repository zu bedienen — die beiden Stellen müssen aber synchron bleiben, sonst entsteht eine Lücke
