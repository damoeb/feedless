...
- [ ] **Übrige GraphQL-Field-Resolver auf den erfassten Aufrufer umstellen.** Verschachtelte Field-Resolver starten unter `Dispatchers.Unconfined` verzögert auf einem Worker-Thread ohne Security-Context. Der Repository-Teilbaum (`RepositoryResolver`, `DocumentResolver`, `AnnotationResolver`) nutzt deshalb `dfe.requestContext()` aus `CallerContextInstrumentation`. `User`, `Order`, `Product`, `License`, `Group` und `Secrets` lesen noch den Thread-Kontext, und `@PreAuthorize` an `User.secrets` sieht keinen Aufrufer, wenn der Eltern-Resolver den Thread gewechselt hat.
- [ ] **`Annotations.votes` liefert immer `[]`.** `DgsCustomContext.userId` wird nirgends gesetzt; stattdessen `dfe.requestContext().userId` verwenden.
...
- [ ] **401-Antworten von `/api/v1` ohne `oauth` tragen keine `x-corr-id`.** `HttpApiJwtFilter` sitzt in der Security-Chain und schickt den 401 selbst per `sendError`, bevor die als Servlet-Filter für alle Pfade registrierte `JwtRequestFilter` überhaupt läuft; die Id wird danach zwar gewählt und gespeichert, aber nicht mehr in die schon gesendete Antwort geschrieben.
...
- [ ] **Antwortzeit verrät geleakte private UUIDs.** Für eingeloggte Fremde kostet eine private Repository-ID mehr Abfragen (Repository, Mitgliedschaften) als eine fehlende (nur Repository), und `/attachment/{id}` lädt die Bytes vor der Prüfung; mit bekannter UUID lässt sich die Existenz so bestätigen, eine Aufzählung ist es nicht
- [ ] **`sourcesCount`/`sourcesCountWithProblems` in `RepositoryFragment` kosten pro Zeile eine Repository- und Benutzer-Abfrage, bei Nicht-Besitzern zusätzlich eine Mitgliedschafts-Abfrage.** Eine Liste von 50 Repositories addiert damit rund 200 zusätzliche Abfragen; den Zugriffsentscheid pro Repository einmal auflösen (z. B. im DataFetchingEnvironment memoisieren) statt pro Feld erneut
...
- [ ] **`RepositoryAccessGuard.requireCallerScope` und `requireRepositoryConfiguration`/`mayAccess` antworten einem gesperrten Aufrufer unterschiedlich.** Die Quellenliste über alle Repos (`requireCallerScope`) meldet seit dem I1-Fix 404 wie ein Fremder; der Zugriff auf ein einzelnes Repository (`requireRepositoryConfiguration`/`mayAccess`) meldet einem gesperrten Mitglied weiterhin 400 „denied", weil die Mitgliedschaft dort schon bestätigt ist, bevor `UserGuard` prüft (bewusst so, siehe `RepositoryAccessRule.isActiveOwnerOrMember`) — die beiden Antworten aufeinander abstimmen oder die Abweichung dokumentieren
...
- [ ] **`DateTimeExtractorTest` (ehemals `DateClaimerTest`) schlägt sporadisch fehl.** In CI-Lauf 34713539900 (Versuch 1) wurde „12 Dezember 24" zu einem Datum im Jahr ~282 statt 2024-12-12; derselbe Commit war beim Neustart grün, lokal ist der Test auch mit `TZ=UTC` grün. Jeder Testfall baut einen eigenen `DateTimeExtractor`, JUnit läuft nicht parallel — Ursache offen (Verdacht: Reihenfolge der Formate in `guessDateFormats` oder das JDK auf CI)
...
- [ ] **Grafana-Dashboard für Fetch-Ergebnisse.** Warum ein Harvest scheitert (Redirect-Schleife, Timeout, 429/Cooldown, DNS, TLS), lässt sich heute nur per `kubectl logs` und `grep` auszählen; es fehlt eine Metrik wie `app_http_fetch_total{via="http|agent", outcome="…"}` (erhöht in `HttpService.execute` und bei den Agent-Antworten in `ScrapeService`) samt Panel in `k8s/grafana/dashboards/site-reliability.json`. Pro Host gehört die Auswertung nach Loki, nicht als Prometheus-Label — sonst wächst die Zahl der Zeitreihen unbegrenzt. Die Outcome-Zuordnung braucht es ohnehin, um aussichtslose Fehler nicht zu wiederholen
