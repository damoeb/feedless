...
export class UpcomingProductPage implements OnInit, OnDestroy {
  ...
  private fillCalendar() {
    ...
    const isToday = (day: Dayjs): boolean => {
      return this.now.isSame(day, 'day') && this.now.isSame(day, 'month') && this.now.isSame(day, 'year');
    }
  ...
  getMonth(month: unknown): string {
    return dayjs().set('month', parseInt(`${month}`)).format('MMMM')
  }
  ...
  private initGeolocation() {
    navigator.geolocation.getCurrentPosition(this.handleLocation, console.error);
  }
