...
class Seeder(
  ...
  private suspend fun seedGroups(root: User) {
  ...
  private suspend fun resolveGroup(groupName: String, owner: User): Group {
    return groupRepository.findByName(groupName) ?: createGroup(groupName, owner)
  }

  private suspend fun createGroup(groupName: String, owner: User): Group {
  ...
  private suspend fun seedRootUser(): User {
...
//  }
  ...
  private suspend fun createAnonymousUser() = createUser(
...
//    user.planId = planDAO.findByNameAndProduct(plan, ProductName.system)!!.id
  ...
  private suspend fun seedProducts(root: User) {
