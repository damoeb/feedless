...
//  private lateinit var analyticsService: AnalyticsService
    ...
    suspend fun create(data: List<RepositoryCreateInput>): List<Repository> {
        ...
        val totalCount = withContext(Dispatchers.IO) {
            repositoryDAO.countByOwnerId(ownerId.uuid)
        }
...
//        }
    ...
    suspend fun findById(repositoryId: RepositoryId): Optional<Repository> {
        return withContext(Dispatchers.IO) {
            repositoryDAO.findById(repositoryId.uuid).map { it.toDomain() }
        }
    }
...
//  }
    ...
    suspend fun delete(repositoryId: RepositoryId) {
        withContext(Dispatchers.IO) {
            ...
            if (repository.ownerId != coroutineContext.userId().uuid) {
    ...
    suspend fun updateRepository(id: RepositoryId, data: RepositoryUpdateDataInput) {
        ...
        val repository = withContext(Dispatchers.IO) {
            repositoryDAO.findById(id.uuid).orElseThrow { NotFoundException("Repository $id not found") }
        }
        val corrId = coroutineContext.corrId()
        if (repository.ownerId != coroutineContext.userId().uuid) {
            throw PermissionDeniedException("not authorized")
    ...
    fun countAll(userId: UserId?, product: Vertical): Int {
        return userId
            ?.let { repositoryDAO.countAllByOwnerIdAndProduct(it.uuid, product) }
