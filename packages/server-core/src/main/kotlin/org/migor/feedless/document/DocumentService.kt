...
class DocumentService(
  ...
  suspend fun findAllByRepositoryId(
    ...
    return withContext(Dispatchers.IO) {
      ...
      documentRepository.findAllWithAttachmentsByIdIn(q.resultList.map { DocumentId(it) })
    }
  }
    ...
    val query = jpql {
      ...
      selectNew<Pair<Long, Long>>(
        ...
        .whereAnd(
          ...
          path(DocumentEntity::repositoryId).eq(where.repository.uuid),
          path(DocumentEntity::publishedAt).lt(LocalDateTime.now()),
          *whereStatements.toTypedArray()
        )
        .groupBy(dateGroup)
