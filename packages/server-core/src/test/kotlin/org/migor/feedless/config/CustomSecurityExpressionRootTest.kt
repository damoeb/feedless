...
class CustomSecurityExpressionRootTest {
  ...
  fun `hasCapability returns true when capability exists`() = runTest {
    ...
    val expressionRoot = CustomSecurityExpressionRoot(authentication)
  ...
  @Test
  fun `filter and return object methods work correctly`() = runTest {
    // given
    val authentication = mock(Authentication::class.java)
    `when`(authentication.isAuthenticated).thenReturn(true)
    val expressionRoot = CustomSecurityExpressionRoot(authentication)
    ...
    // when
    val filterObj = "test-filter"
    val returnObj = "test-return"
    expressionRoot.setFilterObject(filterObj)
    expressionRoot.setReturnObject(returnObj)

    // then
    assertThat(expressionRoot.filterObject).isEqualTo(filterObj)
    assertThat(expressionRoot.returnObject).isEqualTo(returnObj)
    assertThat(expressionRoot.getThis()).isEqualTo(expressionRoot)
  }
