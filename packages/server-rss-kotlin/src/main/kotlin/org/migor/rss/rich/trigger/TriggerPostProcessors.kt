...
import org.migor.rss.rich.plugins.ShellCommandPlugin
import org.migor.rss.rich.plugins.YtArchiverPlugin
...
@Service
class TriggerPostProcessors internal constructor() {

  private val log = LoggerFactory.getLogger(TriggerPostProcessors::class.simpleName)
...
//  lateinit var graphService: GraphService
  ...
  @Autowired
  lateinit var shellCommandPlugin: ShellCommandPlugin

  @Autowired
  lateinit var ytArchiverPlugin: YtArchiverPlugin
...
//  @Scheduled(fixedDelay = 15000)
  ...
  private fun handleBucket(bucket: Bucket) {
    try {
      log.info("PostProcessing bucket ${bucket.id}")
  ...
  private fun applyPostProcessor(article: Article, postProcessor: ArticlePostProcessor, bucket: Bucket) {
    try {
      when (postProcessor.type) {
        PostProcessorType.SHELL_COMMAND -> shellCommandPlugin.process(article, bucket, postProcessor.context)
        PostProcessorType.YT -> ytArchiverPlugin.process(article, bucket, postProcessor.context)
      }
    } catch (e: Exception) {
