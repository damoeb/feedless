...
describe('stripHtml', () => {
  ...
  it('keeps the text around the highlight', () => {
    expect(stripHtml('<b>Zug</b> (ZG) - Steinhausen,Baar,Zug')).toBe(
      'Zug (ZG) - Steinhausen,Baar,Zug',
    );
  });
  ...
  it('keeps a slash in a bilingual name intact', () => {
    expect(stripHtml('<b>Biel/Bienne (BE)</b>')).toBe('Biel/Bienne (BE)');
  });
