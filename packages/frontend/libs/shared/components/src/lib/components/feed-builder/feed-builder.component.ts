...
import { ApolloAbortControllerService, ScrapeService } from '@feedless/data-access';
import { ServerConfigService, SessionService } from '@feedless/data-access-auth';
...
import { SearchbarComponent } from '@feedless/ui';
import { RemoveIfProdDirective } from '@feedless/ui';
import { SourceBuilder } from '@feedless/data-access';
...
export class FeedBuilderComponent implements OnInit, OnDestroy {
  ...
  async createOrRefineFeed(refine: boolean, draft = false) {
  ...
  async showTagsModal() {
    this.tags = await this.modalProvider.openTagModal(TagsModalComponent, {
      tags: this.tags || [],
    });
    this.changeRef.detectChanges();
  }
  ...
  async showLocationPickerModal() {
    this.geoLocation = await this.modalProvider.openSearchAddressModal(
  ...
  async createFeedUrl(): Promise<string> {
    if (this.serverConfigService.isSaas()) {
      const token = await this.sessionService.requestAnonymousFeedToken(
        this.getRequestInputUrl(),
      );
