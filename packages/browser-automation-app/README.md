...
The minimal way to run an agent, you need a secret key of a user. An agent on a host the core trusts may omit `APP_SECRET_KEY`; until the core accepts keyless agents, it still rejects them.
...
| APP_SECRET_KEY               | string  | no       | key of user; omit on a trusted host |                           |
