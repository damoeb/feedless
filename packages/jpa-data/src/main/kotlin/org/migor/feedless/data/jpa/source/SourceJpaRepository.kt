...
class SourceJpaRepository(private val sourceDAO: SourceDAO, private val entityManager: EntityManager) :
  ...
  override suspend fun save(source: Source): Source {
    return withContext(Dispatchers.IO) {
      sourceDAO.save(source.toEntity()).toDomain()
    }
  }

  override suspend fun deleteAllById(ids: List<SourceId>) {
    withContext(Dispatchers.IO) {
  ...
  override suspend fun findById(id: SourceId): Source? {
    return withContext(Dispatchers.IO) {
      sourceDAO.findById(id.uuid).getOrNull()?.toDomain()
    }

  }

  override suspend fun saveAll(sources: List<Source>): List<Source> {
    return withContext(Dispatchers.IO) {
      sourceDAO.saveAll(sources.map { it.toEntity() }).map { it.toDomain() }
    }
  }

  override suspend fun findAllByRepositoryIdFiltered(
    repositoryId: RepositoryId,
