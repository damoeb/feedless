...
- **Repository access is one rule** (`RepositoryAccessGuard`), applied to `/repositories/{repositoryId}` and everything below it. The owner and members of the owning group may read. Anyone authenticated may `GET` a `public` repository and its sources, harvests and records, but not change them. A denied repository answers `404 NOT_FOUND` exactly like a missing one, never `403`, so a UUID does not reveal that it exists; a missing record or source answers exactly like one of another repository.
- **Writes are narrower than the guard.** The guard lets the owner and group `owner`/`editor` members through to writes (a group `viewer` only reads), but server-core still restricts some of them: repository update and delete are owner-only (a group editor gets `400` on update, `403` on delete), and source update and delete require the repository's group to be the caller's default group (`400` otherwise). Aligning server-core with the guard is a tracked follow-up.
