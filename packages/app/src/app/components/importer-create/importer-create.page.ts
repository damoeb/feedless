...
import { FeedDiscoveryResult, FeedService, SearchFeedsResponse, TransientGenericFeed, TransientNativeFeed } from '../../services/feed.service';
import { ModalController, ToastController } from '@ionic/angular';
import { ImporterService } from '../../services/importer.service';
import {
  PreviewTransientNativeFeedComponent, PreviewTransientNativeFeedComponentProps
...
export class ImporterCreatePage implements OnInit, ImporterCreatePageProps {
  ...
  existingFeeds: SearchFeedsResponse;
  ...
  async searchFeeds(query: string) {
    if (query?.trim().length > 3) {
      ...
      this.existingFeeds = await this.feedService.searchNativeFeeds(query);
  ...
  async importTransientNativeFeed(
    feed: TransientNativeFeed
  ) {
