...
5. **NEVER modify a shipped Flyway migration.** `packages/server-core/src/main/resources/db/migration/` runs to the version pinned in `spring.flyway.target` (`application-database.yaml`, currently `V87`); changes MUST be additive with the next free `V<n>__` number, and a new migration MUST raise that target in the same commit. **Why:** Flyway checksums applied migrations — an edit breaks every environment that already ran it, and it cannot be repaired forward; a migration added without raising the target is validated but never applied.
