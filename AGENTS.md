...
5. **NEVER modify a shipped Flyway migration.** Every migration in `packages/jpa-data/src/main/resources/db/migration/` runs on the next start; changes MUST be additive with the next free `V<n>__` number. **Why:** Flyway checksums applied migrations — an edit breaks every environment that already ran it, and it cannot be repaired forward.
...
| `frontend` | Nx 23 workspace, Angular 22. Apps `upcoming`, `feed-reader`, `auction-alert`; libs `@feedless/{components,core,geo,graphql-api,guards,testing}`. **npm.** | Gradle → npm |
